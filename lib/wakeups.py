    ...
    for snapshot in candidates(board):
        ...
        try:
            ...
            prompt = (f"Scheduled wakeup for card {card}. Receipt: {token}.\n"
                      ...
                      "An optional draft does not create an operator task. Once actual work is verified complete, "
                      "close the matter and clear triggers without waiting for optional thanks or someone else's "
                      "alert housekeeping. Retain it only for a concrete remaining obligation, risk or verification.\n"
